...
if module == "AI Triage System & Drone Simulation":
    ...
    # Drone Route Simulation with Animation
    ...
    for i, coord in enumerate(route):
        ...
        folium.Marker(route[0], popup="Start Point").add_to(m)
        folium.Marker(route[-1], popup="Destination").add_to(m)
        folium.PolyLine(route[:i+1], color="blue", weight=3).add_to(m)
        folium.Marker(coord, popup=f"Drone Position {i+1}", icon=folium.Icon(color='red')).add_to(m)
        
        # Display map
        map_container.st_folium(m, width=700, height=450)
        time.sleep(1)
...
# ---------------- VITALS MONITORING ----------------
    ...
    fig = px.line(
        x=list(range(10)),
        y=[random.randint(60,100) for _ in range(10)],
        labels={"x":"Time", "y":"ECG"}
    )
